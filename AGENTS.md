...
- **Contrato SSE** (Python→.NET→frontend): eventos en orden `meta → agent* → token* → done → verified? → revision_available?` (+ `error`). `agent` (progreso del Director: planificacion/buscando/hallazgo) es passthrough sin persistencia y SIEMPRE antes del primer token. `done` lleva `{content, sources[], trace, clarify?, media?}` (`clarify={opciones:[{texto,valor}]}` solo en primera pregunta ambigua sin contexto; `media=[{url,tipo,proveedor?,contexto?,documento?,pagina?,seccion?}]` con URLs de los chunks citados); `verified` puede traer `{revision}`.
